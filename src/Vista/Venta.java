...
/* 461 */       total();
/*     */     } else {
/* 463 */       float precio = Float.parseFloat(this.table.getValueAt(fila, 7).toString());
/* 464 */       float subtotal = cantidad * precio;
/* 465 */       this.table.setValueAt(Float.valueOf(subtotal), fila, 8);
/* 466 */       total();
/*     */     }
/*     */   }
/**
 ...
 */
            ...
            for (Object o : tab) {
                ...
                if (x.get(0).toString().equals("Combo")||x.get(0).toString().equals("traseros")||x.get(0).toString().equals("delanteros")) { 
                    if (new mVentas().insertRowSale(x, id)) {
                        ...
                        new mProductos().updateCombo(x.get(9).toString(),jComboBox1.getSelectedItem().toString());
                    } else {
    ...
    private void initComponents() {
        ...
        jTable1.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Producto", "Partida", "K Etiqueta", "# Piezas", "Costo", "Cantidad", "Merma", "Precio Venta", "Total", "idCombo"
            }
        ) {
            ...
            Producto p = (Producto) jComboBox4.getSelectedItem();
             switch(p.getName()){
                ...
                default:           Producto p2 = (Producto) jComboBox4.getSelectedItem();
                                   table.addRow(new Object[]{p2.getName(), "0", "0", "0", "0", new mEntradas().ultimoCosto(p.getName()),"0", "1", "0","0"});
                                   total();
                break;
            }
        }
...
private void loadTable(ArrayList da) {
        for (Object o : da) {
            ...
            table.addRow(new Object[]{r.get(0).toString(),r.get(1).toString(),r.get(2).toString(),r.get(3).toString(),r.get(5).toString(),r.get(6).toString(),r.get(4).toString(),"0","0",r.get(7).toString()});
            total();
            partida = partida + 1;
        }
    }
...
private Double total() {
        ...
        for (int i = 0; i < this.jTable1.getRowCount(); i++) {
            if (table.getValueAt(i, 0).equals("Combo")) {
                ...
                float precio = Float.parseFloat(this.table.getValueAt(i, 7).toString());
...
private boolean validar(){
    boolean b=true;
    for (int i = 0; i < this.jTable1.getRowCount(); i++) {
            Double d=Double.parseDouble(table.getValueAt(i, 7).toString());
            if(d<10){
            b=false;
            }
}
